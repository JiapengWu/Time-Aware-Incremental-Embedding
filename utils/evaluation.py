...
class EvaluationFilter:
    ...
    def calc_metrics_single_graph(self, ent_mean, rel_enc_means, all_ent_embeds, samples, id_dict, time, eval_bz=100):
        with torch.no_grad():
            s = samples[:, 0]
            r = samples[:, 1]
            o = samples[:, 2]
            ...
            # pdb.set_trace()
            o_mask = self.mask_eval_set(samples, test_size, num_ent, time, id_dict, mode="tail")
            s_mask = self.mask_eval_set(samples, test_size, num_ent, time, id_dict, mode="head")
            # perturb object
            ranks_o = self.perturb_and_get_rank(ent_mean, rel_enc_means, all_ent_embeds, s, r, o, test_size, o_mask, id_dict, eval_bz, mode='tail')
            # perturb subject
            ranks_s = self.perturb_and_get_rank(ent_mean, rel_enc_means, all_ent_embeds, s, r, o, test_size, s_mask, id_dict, eval_bz, mode='head')
            ranks = torch.cat([ranks_s, ranks_o])
            ranks += 1 # change to 1-indexed
            # print("Graph {} mean ranks {}".format(time.item(), ranks.float().mean().item()))
        return ranks

    def perturb_and_get_rank(self, ent_mean, rel_enc_means, all_ent_embeds, s, r, o, test_size, mask, id_dict, batch_size=100, mode ='tail'):
        ...
        for idx in range(n_batch):
            ...
            target = torch.tensor([id_dict[i.item()] for i in target])
    ...
    def mask_eval_set(self, test_triplets, test_size, num_ent, time, id_dict, mode='tail'):
        time = int(time.item())
        ...
        for i in range(test_size):
            ...
            if mode == 'tail':
                ...
                tail_idx = np.array(list(map(lambda x: id_dict[x], tails)))
                mask[i][tail_idx] = 1
                mask[i][id_dict[t]] = 0
            elif mode == 'head':
                heads = self.true_heads[time][(r, t)]
                head_idx = np.array(list(map(lambda x: id_dict[x], heads)))
                mask[i][head_idx] = 1
                mask[i][id_dict[h]] = 0
            # pdb.set_trace()
        return mask.byte()
