...
class CorruptTriples:
    ...
    def single_graph_negative_sampling(self, triples, t, g, num_ents):
        neg_tail_sample, neg_head_sample, label = self.negative_sampling(self.true_heads_train[t], self.true_tails_train[t], triples, num_ents, g)

        neg_tail_sample, neg_head_sample = torch.from_numpy(neg_tail_sample), torch.from_numpy(neg_head_sample)
        if self.use_cuda:
            neg_tail_sample, neg_head_sample, label = \
                cuda(neg_tail_sample, self.args.n_gpu), cuda(neg_head_sample, self.args.n_gpu), cuda(label, self.args.n_gpu)
        return neg_tail_sample.long(), neg_head_sample.long(), label

    def negative_sampling(self, true_head, true_tail, triples, num_entities, g):
        size_of_batch = triples.shape[0]
        ...
        return neg_tail_samples, neg_head_samples, labels
